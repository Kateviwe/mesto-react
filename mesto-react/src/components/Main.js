        ...
        <main className="content">
            <section className="profile">
                <div className="profile__wrapper">
                    ...
                    <button className="profile__icon-edit" type="button" onClick={props.onEditAvatar}></button>
                </div>
                <div className="profile__info">
                    <div className="profile__local-info">
                        <h1 className="profile__name">{userName}</h1>
                        <button className="profile__button profile__button_purpose_edit" type="button" onClick={props.onEditProfile}></button>
                    </div>
                    <p className="profile__characteristic">{userDescription}</p>
                </div>
                <button className="profile__button profile__button_purpose_add" type="button" onClick={props.onAddPlace}></button>
            </section>
            <section className="photogrid">
                <ul className="photogrid__container">
                {cards.map(card => <Card card={card} nameCard={card.name} likesCard={card.likes} linkCard={card.link} key={card._id} onSelectedCardClick={props.onCardClick} />)}
                </ul>
            </section>
        </main>
    );
